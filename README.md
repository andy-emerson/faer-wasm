...
| recursive LU (runner-tuned) is the fastest wasm LU measured: beats scipy at n ≤ 128 (1.1–1.5×), ~parity at 256, ~0.8× at 512; on wasm the recursion is marginal (3-round sweep: pure-flat wins ≤384, ties at 512) — the flat simd128 panel is the engine | observed | scripted | Run 5/6 same-run three-way; knobs swept 3-round on the runner (`lu-tune.yml`) and gated (`gate.mjs` recursive ≤ blocked ≤ faer-tuned) |
